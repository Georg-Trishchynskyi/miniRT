...
/*   Updated: 2023/01/22 20:14:38 by fstaryk          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double   hyperboloid_intersection(t_p3 d, t_p3 cam_o, t_hyper_b hy)
{
    ...
    quad_kof[0] = (d.x*d.x/param_sq.x)*hy.nv.x + (d.y*d.y/param_sq.y)*hy.nv.y + (d.z*d.z/param_sq.z)*hy.nv.z;
    quad_kof[1] = (2*new_o.x*d.x / param_sq.x)*hy.nv.x + (2*new_o.y*d.y / param_sq.y)*hy.nv.y + (2*new_o.z*d.z / param_sq.z)*hy.nv.z;
    quad_kof[2] = (pow(new_o.x, 2) / param_sq.x)*hy.nv.x + (pow(new_o.y, 2) / param_sq.y)*hy.nv.y + (pow(new_o.z, 2) / param_sq.z)*hy.nv.z - hy.r;
...
t_p3   hyberboloid_normal(t_p3 inter_p, t_hyper_b hy)
{
    ...
    new_o = inter_p;
    param_sq = hy.param;//new_vec(pow(hy.param.x, 2), pow(hy.param.y, 2), pow(hy.param.z, 2));
    normal = new_vec(2*new_o.x/param_sq.x, 2*new_o.y/param_sq.y, -2*new_o.z/param_sq.z);
    return _norm(normal);
}